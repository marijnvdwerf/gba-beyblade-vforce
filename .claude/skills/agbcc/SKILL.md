...
- Entry normalization proves narrowing at entry, not a narrow formal parameter; try a wide ABI parameter assigned or cast to a narrow local. Conversely, a genuinely narrow formal tested for truth can emit the low-byte shift/test at entry without explicit source shifts. Caller-side `lsl #24` alone or `lsl #24; lsr #24` around a returned or tested value can likewise be normalization generated by an `unk8` return prototype, not source shift choreography; declare the narrow return and never transcribe the shifts.
...
- Retain staged coordinate/extrema values when folding changes allocation: measured byte-required classes include signed wide coordinates loaded before delta math whose later `asr` becomes `lsr` if unsigned, root and nested min/max/midpoint locals, a separate `current` node alias during a tree walk, and width/height shift temporaries that preserve prologue or stack-argument offsets.
...
- Straight-line struct initialization preserves lexical store order; chained assignment stores right-to-left and keeps the value live, while splitting it can reverse stores or add a `mov`. Likewise, keep a measured nested allocation assignment such as `(p = alloc())->field = value`: splitting the pointer assignment from the field store can move the allocation result to a different register.
...
- Prototypes affect both sides: parameter widths select call-site conversions, while definition widths control entry normalization and allocation. A measured program can require a wide `s32` declaration visible to callers while the definition uses narrow `s16` parameters; forcing the narrow prototype into every TU can swap call-argument setup. Preserve the proven per-TU view, and keep a residual `(s16)` at a call site when removing it changes the caller's frame or allocation.
...
- Keep dead-looking calls, arithmetic, divisions, readbacks, and copies that asm shows; a dead result may still expose original operation order or constant construction. Open problem: target-only dead compare/range-test/constant-negation blocks can survive even when their result is overwritten immediately, while natural conditional or comma-expression reconstructions are rejected or optimized away. If agbcc eliminates every natural C spelling, park the function rather than force retention with `volatile`, fake guards, or register hacks.

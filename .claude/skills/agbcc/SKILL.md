...
- For a signed halfword sentinel whose target is `ldrh; orr; strh`, write `field |= -1`. A positive `|= 0xFFFF` can optimize away that read-modify-write, while explicit signed read casts become the wrong source shape once the field declaration is corrected.
...
- A shared zero register does not by itself prove a source local; direct `= 0` assignments can CSE to one register. But when separate direct or chained stores change the literal/store sequence, retain one measured `zero` temporary shared by those stores.

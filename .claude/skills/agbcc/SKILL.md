...
- A rotated CFG in the target (entry test + duplicated bottom test) is not always reproducible from the plain `while (cond)`: both `while (cond)` and `for (;;)` with the same body can change regalloc and shorten the CFG. Try the natural form first; when the diff proves it, spell out the entry test and the bottom `if (… ) break;` explicitly.

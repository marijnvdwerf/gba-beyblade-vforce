...
- Declarations precede statements; normally prototype calls, cast pointer↔integer conversions, never dereference `void *`, and keep unused parameters. Preserve a measured legacy cross-TU declaration view when forcing one shared prototype changes bytes.
...
- Keep a measured `(unk8)` conversion on a phase-derived table index when replacing it with `& 0xFF` changes register allocation or raw opcodes despite equivalent decoded operations.
...
- Byte-required: for a signed halfword sentinel whose target is `ldrh; orr; strh`, write `field |= -1`. A plain store or positive `|= 0xFFFF` can optimize away that read-modify-write, while explicit signed read casts become the wrong source shape once the field declaration is corrected.
- Do not retain a dead assignment from a field merely to steer signedness; correct the field type and remove redundant read casts when the rebuilt users remain exact.
...
- Keep duplicated branch/case-local calculations or stores when hoisting changes dispatch liveness, branch layout, or address lifetimes; identical semantics do not imply a shared source block.
...
- Stage pointer arithmetic when asm stages base, displacement, and scaled index separately; a folded expression invites reassociation or indexed loads. Write `base + (offset + constant)` when the target computes one displacement before adding the base; unparenthesized left-to-right addition emits incremental adds.
...
- Straight-line struct initialization preserves lexical store order. A long chain `a = b = c = d = e = value` can materialize every lvalue address first, then store right-to-left while keeping the value live; splitting it changes the address/store schedule or adds moves. Likewise, keep a measured nested allocation assignment such as `(p = alloc())->field = value` when splitting it changes the result register.
...
- Constant spelling matters only when measured: staged forms such as `mask = 1; mask = -mask;` can force `mov`/`neg`, while a direct large literal may use the pool. Conversely, shift-spelled constants such as `(0xC0 << 5)` have compiled identically to the plain literal; use the readable literal when compare proves it byte-neutral.
...
- For bitfield writes, expect allocation-width read-modify-write: mask the input, preserve other bits, merge, and `strb`/`strh`. A halfword preserve mask may appear as `mov #16; neg`. Compound `|=`/`&=` first extracts the field, applies `orrs`/`bics`, remasks, preserves, merges, and stores; it is not equivalent codegen to assignment or a whole-unit operation.
- Declare proven bitfields directly in the containing struct and name unknown fields `unk<BYTE_OFFSET>_<START_BIT>`. A wrapper struct or union around byte bitfields can be four bytes under agbcc and shift following fields; verify `sizeof` and offsets rather than assuming the wrapper is byte-sized.
...
- agbcc CSEs loads, merges shifts/blocks, and reuses related constants. Signed `s32` arithmetic can keep `a * 2 + b * 2` as two shifts where unsigned `unk32` permits `(a + b) * 2`; use the measured signedness when the target stays unfolded.
...
- Identical case bodies may merge; an unsigned normalized dispatch expression can sometimes restore the expected table form. Conversely, write measured identical tails in the separate case arms and let agbcc cross-jump them naturally; factoring the tail or adding a source `goto` can change allocation and layout.
...
- If one global aggregate supplies several nested subobject addresses across calls, cache its pointer and any proven subobject pointers; repeated chains alter reloads and saved-register lifetimes. A cached pointer to an embedded global subobject can also be required solely to preserve separate `global + memberOffset` base formation plus a later field displacement instead of one folded interior literal.
...
- Later shifts, masks, or division do not narrow a field's memory access. If assembly proves the same storage is accessed at incompatible widths or carries distinct payload interpretations, model one documented union: whole/parts members for scalar widths, typed pointer variants for one pointer slot, or named protocol/state payload members for a shared block. Extend an existing canonical value union with proven typed payload members instead of creating duplicate view structs.
...
- For callbacks whose epilogue preserves `r0`, test a narrow non-void definition that falls off the end with no `return` statement; changing it to `void` can select a different pop register even when the body is otherwise identical.
- Keep dead-looking calls, arithmetic, divisions, readbacks, and copies that asm shows; a dead result may still expose original order or constant construction. A natural conditional transform can leave its arithmetic after the condition and assignment optimize away, so test ordinary clamps or absolute-value shapes before declaring a target-only dead block irrecoverable. If every natural spelling disappears, park rather than use `volatile`, fake guards, or register hacks.

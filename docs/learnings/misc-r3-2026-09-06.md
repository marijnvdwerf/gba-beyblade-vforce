...
The function returns a spline line record from the variable-length line area following a spline's point-index array. `GeometrySplineLine` is a 0x10-byte record with only `unkC` currently named because that is the field used by its caller. The index parameter and count local must be signed to emit the target's `bge`; using `unk32` produced `bhs` at the bounds check. The exact C implementation used `&spline->pointIndices[count]` as the dynamic line-area base and typed indexing for the 0x10-byte records, but that overlay cast is prohibited. The function is parked with its assembly include restored because no compliant typed member/access path can express the variable line-area base.

The single-use `lines` alias was folded into the indexed return during the geometry probe. The first instruction diverged at `+0x00` and the function became four bytes shorter, so the alias-based draft remains the best measured draft.
...
| --- | --- | ---: |
| Baseline parked typed draft | `+0x12` | 0 bytes |
| Fold `mask` into the `REG_IF` write | `+0x08` | -4 bytes |
| Write `REG_IF` directly instead of using `irq` | `+0x00` | -4 bytes |
...
Short-circuiting the duplicate table and row null guards changed the target at `+0x20`; the separate guards are retained.
...
Folding `Div(seconds, 60)` directly into the first `showNumber` call produced an exact instruction match, so the `minutes` temporary was removed.
...
| Change | First divergent instruction | Size delta |
| --- | --- | ---: |
| Baseline typed `data->entries` cursor | `+0x12` | 0 bytes |
| Typed cursor from `gameData->collectables.entries` | `+0x0E` | 0 bytes |
| Typed record-alias cursor `&record->entries[0]` | `+0x0E` | 0 bytes |
| Fold `GetLineMetaData` into the metaobject call | `+0x0E` | 0 bytes |
| Cursor probe with missing `sub_8057104` declaration | build failure; no diff | n/a |

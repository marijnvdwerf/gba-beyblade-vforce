...
- After `GetStruct4` was removed, `uv run tools/worklist.py` contained no other `src/gameinit.c` leaves. The coordinator explicitly authorized three additional small gameinit leaves despite their lack of current C callers: `sub_8053920`, `sub_8053954`, and `sub_80540C8`; all matched using existing typed fields or scalar arguments without speculative layouts.

...
Matched; layout accepted by user ruling 2026-09-06. `GeometrySplineLine` is a 0x10-byte record, and the line records start immediately after the variable-length `pointIndices[pointCount]` array. The implementation uses the accepted overlay cast `lines = (GeometrySplineLine*)&spline->pointIndices[count]`.

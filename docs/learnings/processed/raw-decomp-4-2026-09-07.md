...
## Group 9 TalkingHead typedef placement

Moved the `TalkingHead` typedef in `src/beyblade.h` below both `common.h` and
`ram.h` includes. The header dependency order is now consistent with the
other declarations, while `GetTalkingHead` remains unchanged. Its complete
diff is exact and the full ROM compare passes with SHA1
`cd527c8c24e20e33913fc45199e64b3e6138a6e5`.

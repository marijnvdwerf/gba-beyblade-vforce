...
- Batch of four smallest yellows (luna, one each, no park commits):
  ...
  **719 C / 306 asm / 70%.** Queue keeps running, one yellow per agent,
  ...
  handover commit (amended). **726 C / 299 asm / 71%.** fails: renderActor, sub_805E8D8, sub_8060C1C, initRider, sub_80526C8, sub_804EA88, sub_8056610, sub_80627F0, freeSpriteVramLocation, sub_804DDF8, resizeSpriteBlock. Skill fold merged (25 files archived; the fold first wrote bare scopes / duplicate aliases / pointer-comparison rewrites as PERMITTED levers — inverted, corrected before merge; the classifier blocks the fold agent's `git mv`, manager moves the files). Running: sub_804EA88 (particle),
  sub_80510FC (gamestate), sub_8063220 (palette), sub_8065AA0 (backup),
  sub_8056910 (collision), s_rider_804C4B4 (riderphysics).

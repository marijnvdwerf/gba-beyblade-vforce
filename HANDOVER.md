...
- Leaf G MERGED: initQuadTree, sub_804FF88, newCollisionDataRam;
...
- P1 merged (parked draft only, 6607ab1): LoadSpriteSheet 8 bytes short —
  ...
  `(SpriteSheet*)sourceArg` casts replaced by a typed parameter on main.
...
- Callgraph restored (b02c5d5 merged): 2423af7 two-pass tool + `->field()`
  ...
  (Actor.unkB0, CameraState.callback, MenuState.callback,
  _renderFunctionOffsets) as 🧭 nodes. 🟡 set identical to before (54 names);
  43 unique 🔴 (= old − {copy, handler, unkB0} + sub_80522D4 + render_00–09).
...
Progress: 12/66 TUs done, 444 C functions, 563 INCLUDE_ASM remaining (44%).

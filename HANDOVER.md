...
- 2026-09-24, later — TABLE + STRING MIGRATION COMPLETE (all opus agents,
  ...
  MATH TABLES (ffa3560e): asm/data1.s was math.c's .rodata (.text order layer →
  math → memory). Now C in math.c: ScaleReciprocalTable s16[0x10000] =
  (s16)(65536/(s16)i), [0]=-1; ReciprocalTable u16[0x4000] = round(2^16/i) for
  i<16, round(2^20/i) from 16, clamped; both generated by nested SEQ2..SEQ32768
  macros (agbcc folds the constant expressions; user rule: no incbin, no
  generator scripts, macros OK); SinTable s16[320] = 256·sin(2πi/256) (last 64
  repeat), AcosTable u8[256] = 128·acos((k−128)/128)/π, AcosTableHiRes
...
- Batch of four smallest yellows (luna, one each, no park commits):
  ...
  **719 C / 306 asm / 70%.** Queue keeps running, one yellow per agent,
  distinct TUs. sub_805529C MATCHED by OPUS on retry (616016df; 8 builds — for-body-scoped `record`, `(unk8)` derived table index, `ProjectileSystem.unk28` s16; luna's "r8/r9 swap" did not reproduce — a loop-body scope is real control flow and fixed the allocation). sub_804EA88 MATCHED by OPUS on retry (11b281cb; 7 builds — loop-body `sprite` alias was the register lever, `while (count-- != 0)` with s32 count, Particle s16 fields by ldrsh, ParticleSystem.unkC BGLayer*; fold probes via a scratch TU through tools/agbcc, 4 variants per compile). initEventListeners MATCHED by OPUS on retry (cd5dab4a; 7 builds — VLA with non-const bound, indexed store, and EARLY RETURNS instead of `if (a && b)` nesting flipped the count/max roles by changing the allocno count; sibling idiom from initLevelEnvironmentActors). sub_8056610 MATCHED by OPUS on retry (ac6fb398; 8 builds — the park was a SEMANTIC bug: slope bytes come from `result->unkD/unkE`, not `line`; fixing the field owner fixed the "register swap" via the ref count; RiderBase unk28/unk34/unk1B4 s32, CollisionResult unkC/unkD/unkE typed; the parked sub_80567E4 draft's scratch `s8 unkC` now disagrees with the shared type — reconcile at its unpark). fails: sub_804B7FC, sub_805041C, handleEventListeners, sub_80659F0, actor_8058638, sub_805EBCC, sub_80526C8 (opus too; allocator-priority formula now in docs/learnings/allocator-priority-2026-09-13.md), sub_80539E8, LoadSpriteSheet, sub_804E594. allocateParticleSystem merged (820b2d06; `while (arg1-- != 0)`, s32 count param by the post-decrement lowering, Particle typed, `const SpriteSheet*` through the API; `zero`/`sprite` temps folded, block/bytes/count byte-required). **720 C / 305 asm / 70%.** Running: LoadSpriteSheet (sprite),
  ...
  handover commit (amended). **726 C / 299 asm / 71%.** fails: renderActor, sub_805E8D8, sub_8060C1C, initRider, sub_80526C8, sub_804EA88, sub_8056610, sub_80627F0, freeSpriteVramLocation, sub_804DDF8, resizeSpriteBlock, sub_8056910, s_rider_804C4B4, sub_80561EC. THUMB YELLOW QUEUE EXHAUSTED for this session (every yellow attempted once; matches merged, the rest are allocation walls). Skill fold merged (25 files archived; the fold first wrote bare scopes / duplicate aliases / pointer-comparison rewrites as PERMITTED levers — inverted, corrected before merge; the classifier blocks the fold agent's `git mv`, manager moves the files). Running: sub_804EA88 (particle),
  sub_80510FC (gamestate), sub_8063220 (palette), sub_8065AA0 (backup),
  (none — Thumb queue exhausted).
- ARM bank (src/iwram.c, agbcc_arm -O2; user: a non-matching draft is
  better than nothing — parks allowed here). Scope = functions reached
  from mainLoop: the `📦` data-pointer targets (fastMemoryCopy16ARM 96,
  fastMemoryClear16ARM 100, fastMemoryCopyARM 132, fastMemoryClearARM 136,
